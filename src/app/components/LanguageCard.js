...
export default function LanguageCard({ language, problemCount, href, onClick }) {
  ...
  const CardContent = () => (
    <div className={`relative overflow-hidden bg-gradient-to-br ${brand.gradient} text-white rounded-2xl p-8 hover:scale-[1.02] transition-all duration-300 shadow-lg hover:shadow-2xl cursor-pointer group border-2 ${brand.border} h-44`}>
      ...
      <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -mr-12 -mt-12 transition-transform duration-500 group-hover:scale-125"></div>
      <div className="absolute bottom-0 left-0 w-16 h-16 bg-white/5 rounded-full -ml-8 -mb-8 transition-transform duration-500 group-hover:scale-110"></div>
