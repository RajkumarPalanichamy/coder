...
export default function ProblemStatusCard({
  ...
  const isProblemAnswered = (problemIndex) => {
    if (problems && problems.length > 0 && problems[problemIndex]) {
      return problemStatuses.hasOwnProperty(problems[problemIndex]._id);
    }
  ...
  const isProblemMarked = (problemIndex) => {
    if (problems && problems.length > 0 && problems[problemIndex]) {
      return markedProblems.has(problems[problemIndex]._id);
    }
    return false;
  };
  ...
  const getAnsweredCount = () => {
    if (problems && problems.length > 0) {
      return problems.filter(p => problemStatuses.hasOwnProperty(p._id)).length;
    }
    return answeredCount;
  };

  const activeAnsweredCount = getAnsweredCount();
  const notAnsweredCount = totalProblems - activeAnsweredCount;
