...
function SubmissionsContent() {
  ...
  const handleDetailsClick = (submission) => {
    if (submission.type === 'problem') {
      ...
      // Straight to the read-only review of this attempt - never back into the test itself
      router.push(`/dashboard/tests/${submission.test?._id}/result?submissionId=${submission._id}`);
    }
  };
                ...
                <div className="grid grid-cols-3 gap-4 text-sm text-gray-600">
                  <div>
                    ...
                    <div className="flex flex-col items-center gap-1">
                      {submission.type === 'problem' && submission.language ? (
                        <>
                          {getLanguageImage(submission.language)}
                          <span className="capitalize text-xs">{submission.language}</span>
                        </>
                      ) : (
                        <span className="capitalize">{submission.type === 'problem' ? submission.language : 'Multiple Choice'}</span>
                      )}
                    </div>
                  </div>
                  <div>
